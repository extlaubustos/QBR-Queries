-- description: Métricas de engagement de usuarios por origen, plataforma y clasificación para la semana actual hasta ayer (WTD)
-- domain: behaviour
-- product: mplay
-- use_case: reporting
-- grain: sit_site_id, origin, platform, user_classification, team
-- time_grain: week_to_date
-- date_column: DS
-- date_filter: desde el inicio de la semana hasta ayer
...
-- - BASE_MPLAY_WTD joins CLASSIFICATION_ORIGINS y LK_MPLAY_SOURCE_TYPE_ORIGIN_SESSION por origin
-- owner: data_team
...
-- CTE de Sesiones (Filtro de fecha aplicado aquí para el rango WTD)
SESSIONS AS (
    SELECT
        s.SIT_SITE_ID,
    ...
    WHERE 
        s.ds >= DATE_TRUNC(CURRENT_DATE(), WEEK(MONDAY)) -- 🚨 FILTRO CLAVE: Desde el Lunes (inicio de la semana)
...
-- CTE de Agregación WTD (Agregando al nivel WEEK_ID)
BASE_MPLAY_WTD AS (
    SELECT
        'WTD' AS timeframe_type,
        DATE_TRUNC(s.DS, WEEK(MONDAY)) AS timeframe_id, -- El WEEK_ID es el ID para la carga WTD
        s.sit_site_id,
        DATE_TRUNC(s.DS, MONTH) AS MONTH_ID,
        DATE_TRUNC(s.DS, WEEK(MONDAY)) AS WEEK_ID,
        CASE
