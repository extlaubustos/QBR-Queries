...
CREATE OR REPLACE TABLE `meli-sbox.MPLAY.MPLAY_NEGOCIO_ADQUITSTION_METRICS` AS (
  ...
  -- LEFT JOIN con la tabla de lookup de tipos de origen de sesión para tener el origen y source_session correcto matcheando por el origin_path
...
CREATE OR REPLACE TABLE `meli-sbox.MPLAY.MPLAY_NEGOCIO_COHORTS_DASH` AS (
  WITH PLAY_DAYS AS (
    ...
    SELECT DISTINCT
      ...
      (P.DS) AS DAY_PLAY,
      -- Suma el tiempo de reproducción en minutos
  ...
  TABLE_CALENDAR AS (
    -- CTE para generar un calendario de cohortes mensuales de 30 días
    SELECT
      *
    FROM (
      SELECT
        *,
        -- Asigna un número de mes a cada cohorte de 30 días
      ...
      -- +1 PARA SACAR DIA DE ALTA
        ...
        -- Filtra para obtener solo rangos de 30 días completos
  ...
  BASE AS (
    SELECT
      *,
      ...
      CASE WHEN FLAG_TVM > 0 AND ROW_NUMBER()OVER(PARTITION BY SIT_SITE_ID,USER_ID,FLAG_TVM ORDER BY MONTH_NUMBER ASC) = MONTH_NUMBER THEN 'ALL_MONTH' ELSE 'NOT_RECURRENT' END AS FLAG_CONSEC,
    ...
    FROM (
      SELECT
        ...
        -- Suma el TVM para cada usuario en cada período de 30 días
        SUM(PLAYBACK_TIME) AS TVM,
        -- Flag para indicar si hubo actividad
  ...
  SELECT
    ...
    -- Cuenta el total de usuarios con retención
...
CREATE OR REPLACE TABLE `meli-sbox.MPLAY.MPLAY_NEGOCIO_ATT_AHA_MOMENT_USERS` AS (
  -- CTE para identificar a los usuarios que alcanzaron el AHA
  WITH CONV AS (
    SELECT
      A.USER_ID,
      A.SIT_SITE_ID,
    ...
    WHERE A.DS BETWEEN FIRST_DS_USER AND FIRST_DS_USER+29
    -- Califica para traer solo la fecha en la que el usuario alcanzó su tercer día de reproduccion
    QUALIFY ROW_NUMBER()OVER(PARTITION BY SIT_SITE_ID,USER_ID ORDER BY DS ASC ) = 3
  ),
  -- CTE para atribuir el AHA a los orígenes de sesión
  ATT_ORIGINS AS (
    SELECT DISTINCT
      C.USER_ID,
      C.SIT_SITE_ID,
      ...
      S.ORIGIN_PATH,
      S.DS AS FECHA_ESTIMULOS,
      -- Calcula los días desde la sesión de origen hasta la fecha del AHA
      DATE_DIFF(C.FECHA_CUMPLE_AHA,S.DS,DAY) AS DIAS_TO_CONV
    FROM CONV AS C
    -- LEFT JOIN con BT_MKT_MPLAY_SESSION para encontrar las sesiones que llevaron al AHA
    LEFT JOIN `meli-bi-data.WHOWNER.BT_MKT_MPLAY_SESSION` AS S
      ON S.USER_ID = C.USER_ID
      AND S.SIT_SITE_ID = C.SIT_SITE_ID
      -- Filtra las sesiones que ocurrieron entre el primer play del usuario y la fecha del AHA
      AND S.DS BETWEEN C.FIRST_DS_USER AND C.FECHA_CUMPLE_AHA
      -- AND S.DS >= FECHA_FROM-5-30

  -- CTE base para los cálculos de atribución del AHA
  BASE_CALCULOS AS (
    SELECT
      *,
      -- Asigna un rango denso para ponderar los orígenes de sesión, donde los más cercanos al AHA tienen un peso mayor
      dense_rank()OVER(PARTITION BY USER_ID,SIT_SITE_ID ORDER BY DIAS_TO_CONV DESC) AS ORDER_DESC_TIME --> USAMOS DENSE PARA PONERLE EL MISMO PESO AL DIA
    FROM ATT_ORIGINS
  )
  ...
  SELECT
    *,
    -- Calcula el peso de conversión para el AHA, ponderando los orígenes según su cercanía
    POW(2,ORDER_DESC_TIME)/(SUM(POW(2,ORDER_DESC_TIME))OVER(PARTITION BY USER_ID,SIT_SITE_ID)) AS CONVERTION_W
  FROM BASE_CALCULOS
)