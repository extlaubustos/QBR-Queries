WITH USER_MONTHS AS (
  SELECT
    ...
    UM.BUYER_CATEGORY,
    CASE
      WHEN DATE_DIFF(CURRENT_DATE, UM.LAST_PLAY_DATE, MONTH) <= 6
        THEN DATE_DIFF(CURRENT_DATE, UM.LAST_PLAY_DATE, MONTH)
      ELSE 7
    END AS AGING_MONTHS,
    ...
    DATE_DIFF(
      CURRENT_DATE(),
      CASE
        WHEN UM.FIRST_PLAY_DATE <= DATE '2024-08-01' THEN DATE '2024-08-01'
        ELSE UM.FIRST_PLAY_DATE
      END,
      MONTH
    ) AS total_meses_posible_Actividad,
    COUNT(DISTINCT CASE
      WHEN (SN.SEGMENT_LIFE_CYCLE LIKE '%RETENT%' OR SN.SEGMENT_LIFE_CYCLE LIKE '%EARLY%')
        AND SN.TYPE_USER LIKE 'HIGH%'
        THEN SN.SNAPSHOT_DATE
      ELSE NULL
    END) AS TOTAL_HIGH,
    COUNT(DISTINCT CASE
      WHEN (SN.SEGMENT_LIFE_CYCLE LIKE '%RETENT%' OR SN.SEGMENT_LIFE_CYCLE LIKE '%EARLY%')
...
FROM (
  SELECT
    *,
    SAFE_DIVIDE(
      (TOTAL_HIGH * 3 + TOTAL_MED * 2 + TOTAL_LOW * 1),
      (TOTAL_MESES_POSIBLE_ACTIVIDAD * 3)
    ) AS SCORE_ENG
  FROM USER_MONTHS
);
...
WITH NEW_RET_RECO AS (
  SELECT
    *,
    ...
    CASE
      WHEN LAG(DS, 1) OVER (PARTITION BY SIT_SITE_ID, USER_ID ORDER BY START_PLAY_TIMESTAMP ASC) IS NULL
        THEN 'NEW'
      WHEN DATE_DIFF(
        DS,
        LAG(DS, 1) OVER (PARTITION BY SIT_SITE_ID, USER_ID ORDER BY START_PLAY_TIMESTAMP ASC),
        DAY
      ) <= 30
        THEN 'RETAINED'
      WHEN DATE_DIFF(
...
USER_MONTHS AS (
  SELECT
    ...
    DATE_DIFF(
      RECOVER_DATE,
      CASE
        WHEN SN.FIRST_PLAY_DATE <= DATE '2024-08-01' THEN DATE '2024-08-01'
        ELSE SN.FIRST_PLAY_DATE
      END,
      MONTH
    ) AS total_meses_posible_Actividad,
    COUNT(DISTINCT CASE
