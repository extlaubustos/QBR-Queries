...
PLATFORM_DATA AS (
  SELECT
    ...
    MONTH_ID,
    CASE
      WHEN ROUND(TVM, 2) = ROUND(TVM_CAST, 2) AND TVM_CAST > 0 THEN 'CAST' -- Esto es lo nuevo 
      ELSE CONCAT(
...
SELECT
  ...
  T.CHANNEL,
  P.PLATFORM_CONCAT,
  ROUND(SUM(ATTR_USER), 2) AS TOTAL_USERS,
  ROUND(SUM(CASE WHEN F.FLAG_RET = 1 THEN ATTR_USER ELSE NULL END), 2) AS TOTAL_USERS_RETENTION,
  ROUND(SUM(CASE WHEN F.TOTAL_DAYS_DISTINCT >= 2 THEN ATTR_USER ELSE NULL END), 2) AS TOTAL_USERS_AHA_MOMENT
